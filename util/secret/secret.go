...
// RegenrateAsCore deletes an existing secret created by core CAPI components and recreates a new Secret
// with the Type field correctly set.
func RegenrateAsCore(ctx context.Context, ctrlclient client.Client, existingSecret *corev1.Secret) error {
	if existingSecret.Type == corev1.SecretTypeOpaque {
		newSecret := existingSecret.DeepCopy()
		if err := ctrlclient.Delete(ctx, existingSecret); err != nil {
			return errors.Wrap(err, "failed to delete existing secret")
		}
		newSecret.Type = clusterv1.ClusterSecretType
		if err := ctrlclient.Create(ctx, newSecret); err != nil {
			return errors.Wrap(err, "failed to recreate new secret")
		}
	}
	return nil
}
