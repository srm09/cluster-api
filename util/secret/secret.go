...
// MarkAsCore marks that a secret was generated by core CAPI components
func MarkAsCore(ctx context.Context, ctrlclient client.Client, secret *corev1.Secret) error {
	if secret.Type == "" {
		secret.Type = clusterv1.ClusterSecretType
		if err := ctrlclient.Update(ctx, secret); err != nil {
			return errors.Wrap(err, "failed to update secret type")
		}
	}
	return nil
}
